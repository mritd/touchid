...
int AuthenticateWithBiometrics(char const* reason) {
 LAContext *laContext = [[LAContext alloc] init];
 NSError *authError = nil;
 dispatch_semaphore_t sema = dispatch_semaphore_create(0);
 NSString *nsReason = [NSString stringWithUTF8String:reason];
...
int AuthenticateWithWatch(char const* reason) {
 LAContext *laContext = [[LAContext alloc] init];
 NSError *authError = nil;
 dispatch_semaphore_t sema = dispatch_semaphore_create(0);
 NSString *nsReason = [NSString stringWithUTF8String:reason];
 __block int result = 0;
 if ([laContext canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithWatch error:&authError]) {
   [laContext evaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithWatch
	 localizedReason:nsReason
	 reply:^(BOOL success, NSError *error) {
	   if (success) {
		 result = 1;
...
int AuthenticateWithBiometricsOrWatch(char const* reason) {
 LAContext *laContext = [[LAContext alloc] init];
 NSError *authError = nil;
 dispatch_semaphore_t sema = dispatch_semaphore_create(0);
 NSString *nsReason = [NSString stringWithUTF8String:reason];
 __block int result = 0;
 if ([laContext canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometricsOrWatch error:&authError]) {
   [laContext evaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometricsOrWatch
	 localizedReason:nsReason
	 reply:^(BOOL success, NSError *error) {
	   if (success) {
		 result = 1;
...
func AuthenticateWithBiometrics(reason string) (bool, error) {
	...
	result := C.AuthenticateWithBiometrics(reasonStr)
	switch result {
	case 1:
		return true, nil
	case 2:
...
func AuthenticateWithWatch(reason string) (bool, error) {
	...
	result := C.AuthenticateWithWatch(reasonStr)
	switch result {
	case 1:
		return true, nil
	case 2:
...
func AuthenticateWithBiometricsOrWatch(reason string) (bool, error) {
	...
	result := C.AuthenticateWithBiometricsOrWatch(reasonStr)
	switch result {
	case 1:
		return true, nil
	case 2:
